...
describe("Maturity auditability", () => {
  ...
  it("finalized historical maturity remains reconstructable from audit history alone", async () => {
    ...
    expect(insertEntry.field_changes).toMatchObject({ assessment_id: assessment, maturity_scoring_methodology_id: methodology, status: "draft" });
  ...
  it("every audit entry is correctly attributed to the acting user via auth.uid()", async () => {
    const committedId = await (async () => {
      ...
      try {
        ...
        // maturity_assessments (client engagement data, symmetric
        // can_access_engagement) rather than maturity_domains (Tenant-
        // scoped practice content, requires is_active_tenant_member for
        // INSERT — `user` here only has OrganisationMembership).
        const { rows } = await client.query<{ id: string }>(
          ...
          [engagement, org, tenant, assessment, methodology],
        );
