...
| **Invitation** *(P2B.1)* | A pending grant of client-side access, naming a target email rather than an existing User — the mechanism that gets a real client user their first account, closing the gap every earlier milestone's own fixtures worked around with a raw, out-of-band `auth.users` insert. Either organisation-scoped (`engagement_id NULL` — Client Administrator/Privacy Officer/CXO) or engagement-scoped (Business Owner/IT-CISO/Procurement/Legal); never a practice-side role. Lifecycle is `pending → accepted / revoked` only — `expired` is never stored, always computed from `expires_at`. At most one `pending` row per organisation/engagement/email at a time. Only a SHA-256 hash of the invitation token is ever stored, never the raw value. As of P2B.1 this is schema/lifecycle only — no domain function, Server Action, or acceptance mechanism exists yet (see PROGRESS.md, `docs/P2B_CLIENT_INVITATION_DESIGN.md`). | tenant_id, organisation_id, engagement_id (nullable), invited_email, role_id, token_hash, status, expires_at, invited_by, accepted_user_id (nullable), accepted_at (nullable), revoked_at (nullable) | belongs to Tenant/Organisation, optionally Engagement; references Role; references User via invited_by and (once accepted) accepted_user_id |
